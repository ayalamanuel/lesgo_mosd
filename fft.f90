...
public :: kx, ky, k2
public :: forw, back, forw_big, back_big, back_wave, back_wave2

real(rprec), allocatable, dimension(:,:) :: kx, ky, k2
integer*8 :: forw, back, forw_big, back_big, back_wave,back_wave2
real (rprec), dimension (:, :), allocatable :: data, data_big
real (rprec), dimension (:, :), allocatable :: data_wave_out
complex(rprec), dimension (:,:), allocatable :: data_wave_in
contains
...
allocate( data_wave_in(nx/2+1, ny) )
allocate( data_wave_out(nx, ny) )
...
call dfftw_plan_dft_c2r_2d(back_wave, nx, ny, data_wave_in,                         &
    data_wave_out, FFTW_PATIENT, FFTW_UNALIGNED)

deallocate(data)
deallocate(data_big)
deallocate(data_wave_in)
deallocate(data_wave_out)
