...
!*******************************************************************************
function get_eqm_wall_bot() result(eqm_twall)
!*******************************************************************************
!
! This function provides plane-averaged value of wall stress from EQWM
use types, only: rprec
use param, only : nx, ny
use sim_param, only : eqmxz , eqmyz

implicit none
real(rprec) :: eqm_twall, eqm_txsum, eqm_tysum
integer :: jx, jy
...
do jx = 1, nx
do jy = 1, ny
    eqm_txsum = eqm_txsum + eqmxz(jx,jy)
    eqm_tysum = eqm_tysum + eqmyz(jx,jy)
enddo
enddo

eqm_twall = sqrt( (eqm_txsum/(nx*ny))**2 + (eqm_tysum/(nx*ny))**2  )

end function get_eqm_wall_bot

!*******************************************************************************
function get_wpm_wall_bot() result(wpm_twall)
!*******************************************************************************
!
...
wpm_twall =  sqrt( (wpm_txsum/(nx*ny))**2 + (wpm_tysum/(nx*ny))**2  )

end function get_wpm_wall_bot

!*******************************************************************************
function get_unswpm_wall_bot() result(unswpm_twall)
!*******************************************************************************
!
! This function provides plane-averaged value of wall stress from WPM
use types, only: rprec
...
unswpm_twall =  sqrt( (uns_txsum/(nx*ny))**2 + (uns_tysum/(nx*ny))**2  )

end function get_unswpm_wall_bot


!*******************************************************************************
function get_uLES() result(uLES_wall)
!*******************************************************************************
!
! This function provides plane-averaged value of u_LES from EQWM
use types, only: rprec
use param, only : nx, ny
use sim_param, only : u_LES

implicit none
real(rprec) :: uLES_wall, uLES_sum
integer :: jx, jy

uLES_sum = 0._rprec
do jx = 1, nx
do jy = 1, ny
    uLES_sum = uLES_sum + u_LES(jx,jy)
enddo
enddo

uLES_wall =  uLES_sum/(nx*ny)

end function get_uLES
