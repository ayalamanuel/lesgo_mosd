...
use sim_param, only : RHSx, RHSy, RHSz, cc_z, v_omegaz, u_omegay
use fft
...
real(rprec), save, allocatable, dimension(:,:,:) :: cc_big,                    &
    u_big, v_big, w_big, vort1_big, vort2_big, vort3_big, v_omegaz_big,        &
    u_omegay_big, cc_bigz
...
if( .not. arrays_allocated ) then
   ...
   allocate( cc_bigz( ld_big,ny2,nz ) )
   allocate( v_omegaz_big( ld_big,ny2,nz ) )
   allocate( u_omegay_big( ld_big,ny2,nz ) )
...
if (coord == 0) then
    ...
    !  above the wall (could arguably be 0.25 * w(:,:,2))
       ...
       cc_bigz(:,:,1)=const*(v_big(:,:,1)*(-vort3_big(:,:,1))&
       +0.5_rprec*w_big(:,:,2)*(vort2_big(:,:,jzLo)))

       v_omegaz_big(:,:,1)=const*(v_big(:,:,1)*(-vort3_big(:,:,1)))
       u_omegay_big(:,:,1)=const*(0.5_rprec*w_big(:,:,2)*(vort2_big(:,:,jzLo)))
...
if (coord == nproc-1 ) then  ! channel
    ...
    cc_bigz(:,:,nz-1)=const*(v_big(:,:,nz-1)*(-vort3_big(:,:,nz-1))&
        +0.5_rprec*w_big(:,:,nz-1)*(vort2_big(:,:,jzHi))) 
    
    v_omegaz_big(:,:,nz-1)=const*(v_big(:,:,nz-1)*(-vort3_big(:,:,nz-1)))
    u_omegay_big(:,:,nz-1)=const*(0.5_rprec*w_big(:,:,nz-1)*(vort2_big(:,:,jzHi)))
...
do jz = jz_min, jz_max    !nz-1   ! channel
    ...
    cc_bigz(:,:,jz)=const*(v_big(:,:,jz)*(-vort3_big(:,:,jz))&
        +0.5_rprec*(w_big(:,:,jz+1)*(vort2_big(:,:,jz+1))&
        +w_big(:,:,jz)*(vort2_big(:,:,jz))))

    v_omegaz_big(:,:,jz)=const*(v_big(:,:,jz)*(-vort3_big(:,:,jz)))

    u_omegay_big(:,:,jz)=const*(0.5_rprec*(w_big(:,:,jz+1)*(vort2_big(:,:,jz+1))&
        +w_big(:,:,jz)*(vort2_big(:,:,jz))))
...
do jz=1,nz-1
    ...
    call dfftw_execute_dft_r2c(forw_big, cc_bigz(:,:,jz),cc_bigz(:,:,jz))
    call dfftw_execute_dft_r2c(forw_big, v_omegaz_big(:,:,jz),v_omegaz_big(:,:,jz))
    call dfftw_execute_dft_r2c(forw_big, u_omegay_big(:,:,jz),u_omegay_big(:,:,jz))
    ! un-zero pad
    ! note: cc_big is going into RHSx
    call unpadd(RHSx(:,:,jz),cc_big(:,:,jz))
    call unpadd(cc_z(:,:,jz),cc_bigz(:,:,jz))   
    call unpadd(v_omegaz(:,:,jz),v_omegaz_big(:,:,jz))
    call unpadd(u_omegay(:,:,jz),u_omegay_big(:,:,jz))
 ! Back to physical space
    call dfftw_execute_dft_c2r(back, RHSx(:,:,jz), RHSx(:,:,jz))
    call dfftw_execute_dft_c2r(back, cc_z(:,:,jz), cc_z(:,:,jz))
    call dfftw_execute_dft_c2r(back, v_omegaz(:,:,jz), v_omegaz(:,:,jz))
    call dfftw_execute_dft_c2r(back, u_omegay(:,:,jz), u_omegay(:,:,jz))
