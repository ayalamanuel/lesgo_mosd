...
!*******************************************************************************
subroutine write_wave()
!*******************************************************************************
use param ,only: total_time, wbase
implicit none


open(2,file=path // 'output/wave_spectrum.dat', status='unknown',               &
    form='formatted', position='append')

!! one time header output
if (jt_total==wbase) write(2,*)  'total_time, min(Grad_eta) , max(C_mag)'

write(2,*) total_time, 1, 1
close(2)
...
if (jt_total==wbase) write(2,*)                                            &
    'jt_total, total_time, total_time_dim, turnovers, dt, dt_dim, 1.0,         &
     tau_wall, EQM, WPM, UnsWPM, Cx_max, Cx_min'

!! continual time-related output
write(2,*) jt_total, total_time, total_time_dim, turnovers, dt, dt_dim,        &
    1.0, get_tau_wall_bot(), get_eqm_wall_bot(), get_wpm_wall_bot(),           &
    get_unswpm_wall_bot(), maxval(Cx_wave(:,:)), minval(Cx_wave(:,:))
close(2)
...
!
use param, only : jt_total, dt, total_time
    ...
    if ((floor(total_time) >= pavg_tstart).and.(floor(total_time) <= pavg_tend)) then
        ...
        if (.not.pavg%initialized) then
            if (coord == 0) then
                write(*,*) '-------------------------------'
                write(*,"(1a,i9,1a,i9)")                                   &
                    'Starting running phase averaging from ',               &
                    pavg_tstart, ' to ', pavg_tend
                write(*,*) '-------------------------------'
            end if
            call pavg%init()
        else
...
use param, only : dx, dy, rest_print
...
! Pressure
real(rprec), dimension(:,:,:), allocatable :: pres_real, dpdz_real
...
if(rest_print) then
    ...
    vortz_uv(1:nx,1:ny,lbz:nz) = 0._rprec
    ! Use vorticityx as an intermediate step for performing uv-w interpolation
    ! Vorticity is written in w grid
    vortz_uv(1:nx,1:ny,lbz:nz) = dvdx(1:nx,1:ny,lbz:nz) - dudy(1:nx,1:ny,lbz:nz)
    vortz(1:nx,1:ny,lbz:nz) = interp_to_w_grid( vortz_uv(1:nx,1:ny,lbz:nz), lbz)
    ...
    write(13,rec=4) vortz_uv(:nx,:ny,1:nz)    
    close(13)
#endif

    deallocate(vortx, vorty, vortz,vortz_uv)
...
! Checkpoint phase averaging restart data
if ( pavg_calc .and. pavg%initialized ) call pavg%checkpoint()
...
!*******************************************************************************
use param, only : tavg_calc, pavg_calc
implicit none
...
if (pavg_calc .and. pavg%initialized ) call pavg%finalize()
